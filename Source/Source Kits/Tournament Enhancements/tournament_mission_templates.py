...
################
	...
	(1, 4, ti_once, 
		...
		[
			...
			(try_begin),
				...
				#(neq, ":winning_team", ":player_team"),
				(main_hero_fallen),
				(val_div, reg1, 2), # Reduce cumulative difficulty addition by half if you were knocked out.
			(try_end),
	...
	(ti_on_agent_hit, 0, 0, [
		...
		[
			...
			(assign, reg1, ":damage"),
			(str_store_item_name, s31, ":weapon"), ## TPE 1.5.2 ##
			(display_message, "@Your ally, {s1}, delivers {reg1} damage to {s2}. ({s31})", wp_green),
		]),
	...
	# TRIGGER 19: Reduces damage dealt to "champion" scaled heroes and Lords to subtly improve their performance.
	(ti_on_agent_hit, 0, 0, 
		[(eq, "$g_mt_mode", abm_tournament),], 
		[
			# Trigger Param 1: damage inflicted agent_id
