...
# #Formations Scripts	  
  ...
  ("form_archers", [
	...
	(assign, ":rank", 1), #caba - end (plus all uses of ":rank"/new local variables below)
	(try_for_agents, ":agent"),
		(call_script, "script_cf_valid_formation_member", ":fteam", ":fdivision", ":fleader", ":agent"),
		(agent_set_scripted_destination, ":agent", pos1, 1),
		(try_begin),	#First Agent
		...
		(try_begin),
			...
			(agent_set_slot, ":agent", slot_agent_formation_rank_no, ":rank"),			
  ...
  ("formation_process_agent_move", [
	...
	# (try_begin),
		# (eq, ":rank", 1),
		# (agent_set_slot, ":agent", slot_agent_formation_rank_no, 1),
	# (else_try),
...
# #Player team formations functions
  ...
  ("player_attempt_formation", [
	...
	(store_add, ":slot", slot_team_d0_move_order, ":fdivision"),
	(team_get_slot, ":old_mordr", "$fplayer_team_no", ":slot"),  ##Caba - Added to allow formations to begin while a division is following the player
	(try_begin),
	...
	(try_end),
	(try_begin), ##Caba - Added to allow formations to begin while a division is following the player
		(eq, ":old_mordr", mordr_follow),
		(store_add, ":slot", slot_team_d0_move_order, ":fdivision"),
		(team_set_slot, "$fplayer_team_no", ":slot", mordr_follow), #override script_battlegroup_place_around_leader to maintain following
	(try_end),
  ]),
  ...
  ("player_order_formations", [
	...
	(try_begin), #On hold, any formations reform in new location		
		...
		(try_for_range, ":division", 0, 9),
			...
			(team_set_slot, "$fplayer_team_no", ":slot", ":forder"), ##Caba - Added fix bugs after allowing formations to begin while a division is following the player		
...
# #Utilities used by formations
  ...
  ("agent_fix_division",
   [
	...
	(try_begin),
		...
		(try_begin),
			(this_or_next|ge, ":horse", 0), #for troops that can have horses, but may not be guaranteed them
  ...
  ("store_battlegroup_type", [
	...
	(else_try),
		...
		(try_for_agents, ":cur_agent"),
			...
			# (agent_get_wielded_item, reg0, ":cur_agent", 0),  ###PROBLEMATIC--particularly in determining skirmishers below
			...
			(try_begin),
				...
				(else_try), #Infantry
					...
					(try_for_range, ":i", ek_item_0, ":end"),
						...
						(gt, ":item", 0),
						(item_get_type, ":weapontype", ":item"),
						(eq, ":weapontype", itp_type_polearm),  # bow or crossbow
						(assign, ":end", ek_item_0), #loop Break
					(try_end),
					(try_begin),
  ...
  ("store_battlegroup_data", [
	...
	(try_for_agents, ":cur_agent"),
		...
		(try_begin),
			...
			(item_get_type, ":cur_weapon_type", ":cur_weapon"),
		...
		(else_try),
			...
			(try_begin),	#Division First Rank Weapon Length
				(agent_slot_eq, ":cur_agent", slot_agent_formation_rank_no, 1), #CABA was _ge when it was in_first_rank
	...
	(try_for_range, ":team", 0, 4),
	    ...
	    (try_for_range, ":division", 0, 9),
			...
			#Error Check for no enemy in range - Caba change from limiting agent^2 loop
			(store_add, ":slot", slot_team_d0_closest_enemy, ":division"),
			(team_slot_eq, ":team", ":slot", 0),
			(store_add, ":slot", slot_team_d0_closest_enemy_dist, ":division"),
			(team_set_slot, ":team", ":slot", Far_Away),
  ...
  # script_get_order_destination_for_map by Caba'drin
  # Input: position, team, troop class
  # To deal with "prsnt_battle" flag position bugs, 'back shifting' when F2 Advance/Fall Back orders are used
  ("get_order_destination_for_map", [
	(store_script_param, ":fposition", 1),
	(store_script_param, ":fteam", 2),
	(store_script_param, ":fdivision", 3),
	(init_position, ":fposition"),
	(try_begin),
		...
		(team_get_slot, ":y", ":fteam", ":slot"),
		(neq, ":y", 0),
		#Fix for the map FPM of 1000 (versus formation's 100 FPM) in the Native update_* scripts
		(val_mul, ":x", 10),
		(val_mul, ":y", 10),		
		(position_set_x, ":fposition", ":x"),
		(position_set_y, ":fposition", ":y"),
...
def modmerge_formations_scripts(orig_scripts):
	...
	#Bugfix for Flags in Formations - Caba
	try:
		find_i = list_find_first_match_i(orig_scripts, "update_order_flags_on_map")
		opblock = ScriptWrapper(orig_scripts[find_i]).GetOpBlock()	
		pos = opblock.FindLineMatching((team_get_order_position, pos1, ":player_team", grc_infantry))
		opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_infantry))
		pos = opblock.FindLineMatching((team_get_order_position, pos1, ":player_team", grc_archers))
		opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_archers))
		pos = opblock.FindLineMatching((team_get_order_position, pos1, ":player_team", grc_cavalry))
		opblock.SetLineContent(pos, (call_script, "script_get_order_destination_for_map", pos1, ":player_team", grc_cavalry))
	except:
		import sys
