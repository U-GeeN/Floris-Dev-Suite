...
# # AI with Formations Scripts
  ...
  ("field_ai_start_positions", [
    (assign, ":bp_phase", "$battle_phase"),
	(assign, "$battle_phase", BP_Spawn),
	
	##Caba - taken from "select_battle_tactic" as it might be useful - see also 'defense not an option'. something like that may be necessary for freelancer
	(try_begin),
	...
	(else_try),
		...
		(else_try),
		  (assign, "$ai_team_1", 0),
		(try_end),
		(store_add, "$ai_team_2", "$fplayer_team_no", 2),
	(try_end),
		
	(try_begin),
		(team_get_slot, ":ai_faction", "$ai_team_1", slot_team_faction),
		...
		(call_script, "script_formation_battle_tactic_init_aux", "$ai_team_1", 0),
	(try_begin),
	(try_begin),
		(neq, "$ai_team_2", -1),
		(team_get_slot, ":ai_faction", "$ai_team_2", slot_team_faction),
		(this_or_next|eq, AI_for_kingdoms_only, 0),
		(this_or_next|eq, ":ai_faction", fac_deserters),	#deserters have military training
		(is_between, ":ai_faction", kingdoms_begin, kingdoms_end),
		(call_script, "script_formation_battle_tactic_init_aux", "$ai_team_2", 0),
	(try_end),
	
	(call_script, "script_prebattle_agents_set_start_positions", grc_everyone), #Caba - snap into place
	
	(call_script, "script_formation_end", "$ai_team_1", grc_archers), #undoing archer bits
	(team_give_order, "$ai_team_1", grc_archers, mordr_stand_closer),
	(team_give_order, "$ai_team_1", grc_archers, mordr_stand_closer),
	(try_begin),
		(neq, "$ai_team_2", -1),
		(call_script, "script_formation_end", "$ai_team_2", grc_archers), #undoing archer bits
		(team_give_order, "$ai_team_2", grc_archers, mordr_stand_closer),
		(team_give_order, "$ai_team_2", grc_archers, mordr_stand_closer),
	(try_end),
	
	(assign, "$battle_phase", ":bp_phase"),
   ]),
		...
		(else_try),
			...
			(else_try),	#jockey phase
				...
				(else_try),	#don't outstrip infantry when closing
					...
					(assign, ":move_archers", 1),
				(try_end),
			(else_try), ##Caba addition #Jockey phase, continued, in defensive position
				(neq, "$ai_team_2", -1), #Not just player vs enemy, but 2 AI teams
				(try_begin),
					(eq, "$freelancer_state", 1), #Player is enlisted
					(this_or_next|eq, ":team_no", 1),	#attacker?
					(eq, ":team_no", 3),	            #or ally of attacker?
			        (assign, ":move_archers", 1),
				(else_try),
					(neq, "$freelancer_state", 1), #Player not in party
					(store_add, ":player_ally", "$fplayer_team_no", 2),
					(eq, ":team_no", ":player_ally"),
					(assign, ":move_archers", 1), #player side must take initiative
				(try_end),
			(try_end),
...
#	(store_script_param, ":rel_army_size", 2),
	...
	(else_try),		
		...
		(else_try),
			...
			(try_begin), ##CABA added failsafe else try
			    (store_add, ":slot", slot_team_d0_closest_enemy_dist, grc_infantry),
				(neg|team_slot_eq, ":team_no", ":slot", Far_Away),
				(agent_is_active, ":enemy_agent_nearest_infantry"),
			...
			(else_try),
				(call_script, "script_battlegroup_get_position", Temp_Pos, ":team_no", grc_infantry),
				(call_script, "script_get_nearest_enemy_battlegroup_current_position", Nearest_Enemy_Troop_Pos, ":team_no", Temp_Pos),
				(assign, ":enemy_agent_nearest_infantry_dist", reg0),
				(assign, ":enemy_agent_nearest_infantry_team", reg1),
				(assign, ":enemy_agent_nearest_infantry_div", reg2),
				(store_add, ":slot", slot_team_d0_first_member, ":enemy_agent_nearest_infantry_div"),
				(team_get_slot, ":enemy_agent_nearest_infantry", ":enemy_agent_nearest_infantry_team", ":slot"),
				(agent_is_active, ":enemy_agent_nearest_infantry"),
				(agent_get_position, Nearest_Enemy_Troop_Pos, ":enemy_agent_nearest_infantry"),
			(try_end),
...
# (display_message, "@Time {reg2}: Infantry of team {reg0} making formation {reg1}"),
		...
		(try_begin),
				...
				(try_for_range, ":enemy_division", 0, 9),
					...
					(try_begin),	#threat or target?
						...
						(try_begin),
							...
							(store_add, ":slot", slot_team_d0_order_sp_brace, ":enemy_division"), #CABA
							(team_slot_eq, ":enemy_team_no", ":slot", 0), #CABA #division not spear braced
			...
			(try_begin),
				...
				(neg|team_slot_ge, 1, slot_team_reinforcement_stage, "$g_reinforcement_stage"), #AI_Max_Reinforcements), ##FLORIS #CC
			...
			(else_try),
				...
				(neg|team_slot_ge, 0, slot_team_reinforcement_stage, "$g_reinforcement_stage"), #AI_Max_Reinforcements), ##FLORIS #CC
...
# # Utilities used by AI by motomataru
  ...
  # Output:	destination current position, reg0 with distance, reg1 enemy team, reg2 enemy division
  # Run script_store_battlegroup_data before calling!
	...
	(try_for_range, ":enemy_team_no", 0, 4),
		...
		(try_for_range, ":enemy_division", 0, 9),
			...
			(try_begin),
				...
				(assign, ":distance_to_nearest_enemy_battlegoup", reg0),
				(copy_position, ":bgposition", pos0),
				(assign, ":nearest_enemy_team", ":enemy_team_no"),
				(assign, ":nearest_enemy_division", ":enemy_division"),
			(try_end),
		(try_end),
	(try_end),
	(assign, reg0, ":distance_to_nearest_enemy_battlegoup"),
	(assign, reg1, ":nearest_enemy_team"),
	(assign, reg2, ":nearest_enemy_division"),
  ]),
]
