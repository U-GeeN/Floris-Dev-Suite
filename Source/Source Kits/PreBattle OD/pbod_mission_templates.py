...
from module_mission_templates import deploy_pavise #Floris, called in common_pbod_triggers
...
pbod_common_triggers = [
  ...
  horse_whistle,
  shield_bash, 
  deploy_pavise, #Floris, imported from module_mission_templates
 ] 
...
real_deployment_triggers = [
 (0, 0.9, ti_once, [(mission_cam_set_screen_color, 0xFF000000),],
   [
...
prebattle_orders_triggers = [
 ...
 (0, 1.0, ti_once, [(party_slot_ge, "p_main_party_backup", slot_party_prebattle_num_orders, 1)], [ #was ti_once, adjusted to conditions failure to work around engine problems
		#To fix bugs with Move Forward/Back 10 Paces and Caba Weapon orders
		#these orders are applied separately, after other orders
		(set_show_messages, 0),	 
		(set_fixed_point_multiplier, 100), #bugfix
...
field_ai_triggers = [
  ...
  (0, 0, 2, [(key_clicked, "$key_special_0"),(agent_is_alive,"$fplayer_agent_no"), #spearwall_trigger_2
...
bodyguard_triggers = [
 (ti_after_mission_start, 0, 0, [(party_slot_eq, "p_main_party", slot_party_pref_bodyguard, 1),(neq, "$g_mt_mode", tcm_disguised)], #condition for not sneaking in; to exclude prison-breaks, etc change to (eq, "$g_mt_mode", tcm_default")
   [
    ...
    (set_show_messages, 0),   
    (team_give_order, "$fplayer_team_no", grc_everyone, mordr_follow), #was Division 8 to avoid potential conflicts
...
pbod_battle_triggers = pbod_common_triggers + prebattle_deployment_triggers + prebattle_orders_triggers + caba_order_triggers
...
def modmerge_mission_templates(orig_mission_templates):
	...
	for i in range(len(orig_mission_templates)):
		...
		if( mt_name=="village_raid" or mt_name=="castle_attack_walls_defenders_sally"): #mt that don't use common/custom_battle_check_victory_condition
			trigger_i = MissionTemplateWrapper(orig_mission_templates[i]).FindTrigger_i(1,60,ti_once)
			if ( trigger_i != None ):
				codeblock = TriggerWrapper(orig_mission_templates[i][5][trigger_i]).GetConditionBlock()
				pos = codeblock.FindLineMatching((neg|main_hero_fallen, 0))
				codeblock.InsertBefore(pos,[(this_or_next|party_slot_eq, "p_main_party", slot_party_pref_bc_continue, 1)])
				#Change re-arm interval from ti_once to 0 (using work around to edit immutable tuple)
				orig_mission_templates[i][5][trigger_i] = list(orig_mission_templates[i][5][trigger_i])
