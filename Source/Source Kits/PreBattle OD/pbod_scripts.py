...
scripts = [
 ...
 ("prebattle_set_default_prefs", [ 
	...
	(party_set_slot, "p_main_party", slot_party_pref_siege_charge, 0),
	(party_set_slot, "p_main_party", slot_party_pref_ally_division, 0),
    ]),
...
## Caba'drin Orders Begin
 ...
 ("order_volley_begin_end", [ 
	...
	(store_add, ":volley_order_type", ":begin_end", volley_shift),	  
    ...
    (else_try), 	 
		...
		(try_for_agents, ":agent"),
			...
			(try_begin),
				(team_get_movement_order, ":mordr", ":ordered_team", ":division"),
...
#Shield Bash Script - Xenoargh (Blood & Steel); edited/reworked per comments in the thread, etc, by Caba
 ("cf_shield_bash",
   [  
	...
	(assign, ":victim", -1),
	(assign, ":closest_dist", 100), #Set this to whatever you like- 1 meter radius clears a big section of crowd
	(try_for_agents,":agent"),
   ...
   [
	...
	(try_begin),
		...
		(try_begin),
			...
			(display_message, "@Press {s1},{s2},{s3},{s4} to move the camera and use the mouse or MOVEMENT KEYS to rotate. Hold ZOOM to move faster."),
		(else_try),
			...
			(display_message, "@Press {s1},{s2},{s3},{s4},{s5},{s6} to move the camera around the troop you are following. Hold ZOOM to move faster. Press {s7} or {s8} to cycle through troops. Holding {s10} will cycle through the same team and {s11} will cycle to the opposite team."),
		(try_end),	
   ...
   [
	...
	(try_begin),
		...
		(else_try),
			...
			(assign, ":continue", 1),
		(else_try),
			(party_slot_eq, "p_main_party", slot_party_pref_ally_division, 1),
			(agent_get_party_id, ":party", ":agent"),
