   ...
   (ti_on_presentation_event_state_change, 
    [(store_trigger_param_1, ":object"),
	 ...
	 (else_try),    
	   ...
	   (try_begin),		                                                   # Quit from KeyConfig Presentation
		 (eq, "$g_is_quick_battle", 1),
		 (presentation_set_duration, 0),
	   (try_end),    
	   ##(start_presentation, "prsnt_pbod_preferences"),                      # Return To Preferences
	...
	(ti_on_presentation_run,
       [(store_trigger_param_1, ":cur_time"),
        ...
        (try_begin),
		  ...
		  (try_begin),
		    (this_or_next|key_clicked, key_escape),
		    (key_clicked, "$key_order_9"), #current exit key
...
battle_has_troops_replace = [
		...
		(try_for_agents, ":agent"),
		  ...
		  (this_or_next|eq, ":party", -1), #test - for quick battles
...
def modmerge_presentations(orig_presentations):
	...
	try:
		...
		codeblock.Append([
			...
			(overlay_set_position, reg0, pos1),
			(overlay_set_val, reg0, "$g_presentation_credits_obj_9_alpha"),
			(create_text_overlay, reg0, "@Full PBOD^Features"),
		...
		pos = codeblock.FindLastLine()
		codeblock.InsertBefore(pos, [
		   (else_try),
			 ...
			 (start_presentation, "prsnt_mod_option"),
		   (else_try),
		     (store_add, ":overlay", "$g_presentation_credits_obj_12", 1),
		     (eq, ":object", ":overlay"),
			 (assign, "$g_presentation_credits_obj_9_alpha", ":value"),
			 (try_begin),
				(eq, ":value", 1),
				...
				(party_set_slot, "p_main_party", slot_party_pref_bc_charge_ko, 1),
				(party_set_slot, "p_main_party", slot_party_pref_div_dehorse, 0),
				(party_set_slot, "p_main_party", slot_party_pref_div_no_ammo, 0),
				(party_set_slot, "p_main_party", slot_party_pref_formations, 1), 
				(party_set_slot, "p_main_party", slot_party_pref_wp_prof_decrease, 1),
				(party_set_slot, "p_main_party", slot_party_pref_spo_brace, 1),
				(party_set_slot, "p_main_party", slot_party_pref_spo_skirmish, 1),
				(party_set_slot, "p_main_party", slot_party_pref_spo_volley, 1),
				(party_set_slot, "p_main_party", slot_party_pref_wp_prof_decrease, 1),
			 (else_try),
			    #(call_script, "script_prebattle_set_default_prefs"),
				(call_script, "script_floris_set_default_prefs", 0),
			 (try_end),
		   #(try_end), #in script, going above this
		   ])
	except:
		import sys
