...
def generate_presentation_load_script(_mod_options = mod_options):
	...
	opblock.Append([
		...
		# (try_begin),
			...
			# (call_script, "script_mod_options_get_from_file"),
		# (try_end),
	...
	opblock.Append([                       
		...
		## WINDYPLAINS+ ## - Repeated for Bold Effect
		(create_text_overlay, overlay_var, "@%s" % mod_name + " Mod^Options", tf_center_justify),
		...
		(store_div, reg0, floris_version, 100), #version number
		(store_mod, reg1, floris_version, 100), #revision
		(try_begin),
			(eq, reg1, 0),
		...
		(try_end),		
		(str_store_string, s1, "@Version {s1}"),
		## WINDYPLAINS+ ## - Repeated for Bold Effect
...
def generate_presentation_event_state_change_script(_mod_options = mod_options):
	...
	opblock.Append([                       
		(else_try),
			...
			# (else_try),
				# (eq, "$g_mod_options_sync", 1),
				# (call_script, "script_mod_options_write_to_file"),
			(try_end),
		(else_try),
		...
		## WINDYPLAINS+ ## - Allow player to force message log to reappear.
		(else_try),
			(val_add, ":overlay", 1),
			(eq, ":object", ":overlay"),
			(set_show_messages, 1),
			(display_message, "@Message log has been reset to visible.", gpu_debug),
		## WINDYPLAINS- ##
		(try_end),
	])    
	cur_overlay_index += 1 #???? 
