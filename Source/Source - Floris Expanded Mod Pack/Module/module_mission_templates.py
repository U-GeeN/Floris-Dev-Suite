...
pilgrim_disguise = [itm_he_pla_pri_pilgrim,itm_ar_pla_pri_pilgrimdisguise,itm_practice_staff, itm_we_vae_sword_throw_daggers]
...
custom_commander_camera = ( ### DISABLED- Delete if you'd like
  0, 0, 0, [(neg|main_hero_fallen)], #Fix by Caba'drin: this disables the CC Camera when the current commander has fallen and activates the death cam.
  [
    (get_player_agent_no, ":player_agent"),
    (agent_get_look_position, pos1, ":player_agent"),
    (position_move_z, pos1, "$g_camera_z"),
  ...
  1, 0, 0, [],
   [
    ...
    (try_for_agents, ":agent_no"),
      ...
      (store_attribute_level, ":troop_int", ":troop_no", ca_intelligence), ##CC 1.321
      (store_mul, ":extra_xp", ":troop_int", 3), ##CC 1.321
      ...
      (try_begin),
        ...
        (gt, ":extra_xp", 0),
        (eq, "$g_report_extra_xp_and_wpt",1), ##CC 1.324
        (assign, reg1, ":extra_xp"),
        (display_message, "@You got {reg1} extra experience because of your intelligence."), ##CC 1.321
      (try_end),
      (troop_get_xp, ":troop_final_xp", ":troop_no"),
      (troop_set_slot, ":troop_no", slot_troop_extra_xp_limit, ":troop_final_xp"),
      ## add wpn points
      (store_sub, ":wpn_points_add", ":troop_final_xp", ":troop_extra_xp_limit"),
      (store_skill_level, ":skill_level", skl_weapon_master, ":troop_no"),
      (val_add, ":skill_level", 10),
      (val_mul, ":wpn_points_add", ":skill_level"),
      (val_div, ":wpn_points_add", 20),
      (val_div, ":wpn_points_add", 100),
      (gt, ":wpn_points_add", 0),
      (troop_add_proficiency_points, ":troop_no", ":wpn_points_add"),
      (try_begin),
        (eq, ":agent_no", ":player_agent"),
        (eq, "$g_report_extra_xp_and_wpt",1), ##CC 1.324
        (assign, reg1, ":wpn_points_add"),
        (store_sub, reg2, reg1, 1),
        (display_message, "@You got {reg1} weapon {reg2?points:point}."),
      (try_end),
      ## add wpn points
    (try_end),
   ])

##Floris: Updated from CC 1.321, but disabled in 1.322
#custom_commander_init_troop_backup_hp = (
  #1, 0, 0, [],
   #[
    #(get_player_agent_no, ":player_agent"),
    #(try_for_agents, ":agent_no"),
      #(neq, ":agent_no", ":player_agent"),
      #(agent_slot_eq, ":agent_no", slot_agent_spawned, 0),
      #(agent_is_human, ":agent_no"),
      #(agent_get_troop_id, ":troop_no", ":agent_no"),
      #(try_begin),
        #(troop_is_hero, ":troop_no"),
        #(try_begin),
          #(troop_slot_ge, ":troop_no", slot_troop_backup_hp, 1),
      ...
      #(else_try),
        ...
        #(assign, ":backup_hp", reg0),
        #(val_mul, ":backup_hp", ":hp_times_factor"),
        #(val_div, ":backup_hp", 100),
...
#custom_commander_agent_refill_hp = (
  #1, 0, 0, [], 
  #[
    #(get_player_agent_no, ":player_agent"),
    #(try_for_agents, ":agent_no"),
      ...
      #(try_begin),
        ...
        #(try_begin),
          ...
          #(store_sub, ":lost_hp", ":max_hp", ":agent_hp"),
          #(gt, ":lost_hp", 0),
          #(val_min, ":lost_hp", ":backup_hp"),
          #(val_add, ":agent_hp", ":lost_hp"),
...
#custom_commander_agent_refill_hp_when_knocked_down = (
  ...
  #[
    ...
    #(try_begin),
      ...
      #(try_begin),
        ...
        #(val_min, ":recovered_hp", ":backup_hp"),
        #(agent_set_hit_points, ":agent_no", ":recovered_hp", 1),
        #(val_sub, ":backup_hp", ":recovered_hp"),
        #(troop_set_slot, ":troop_no", slot_troop_backup_hp, ":backup_hp"),
      #(try_end),
      #(try_begin),
        #(le, ":backup_hp", 0),
...
custom_commander_change_weather = (
  ti_before_mission_start, 0, 0, [],
   [
    ...
    (party_slot_eq, "p_main_party", slot_party_pref_wp_prof_decrease, 1), #Floris - Weather effects
    (call_script, "script_weather_affect_proficiency", reg0, reg1), #Floris - Weather effects
   ])
...
custom_commander_npc_raise_proficiency =(
  ...
  [
    ...
    (try_for_range, ":weapon_type", wpt_one_handed_weapon, ":wpt_end"),
      ...
      (val_sub, ":amout", 1),
      (assign, reg3, ":amout"),
	  (try_begin), #Floris 2.52 - begin - for weather proficiency hits
	    (is_between, ":weapon_type", wpt_archery, wpt_firearm), #archery, crossbow, throwing
		(party_slot_eq, "p_main_party", slot_party_pref_wp_prof_decrease, 2), #Active this battle
	    (troop_slot_eq, "$g_player_troop", slot_troop_proficiency_modified, 1), #this troop modified
		(store_sub, ":n", ":weapon_type", wpt_archery),
		(store_add, ":prof_slot", slot_troop_orig_wpt_archery, ":n"), 
		(troop_get_slot, reg1, "$g_player_troop", ":prof_slot"),
		(val_add, reg1, reg2),
	  (else_try), #Floris 2.52 - end	
		(assign, reg1, ":cur_weapon_proficiency"),
	  (try_end), #Floris 2.52 - for weather proficiency hits
...
##Floris: Disabled because these banners don't display the flags right. Furthermore they are not included in the pack.
...
#      (assign, ":cur_banner", reg0),
#      (store_add, ":arms_meshes_end", arms_meshes_end_minus_one, 1),
#      (try_begin),
#        (is_between, ":cur_banner", arms_meshes_begin, ":arms_meshes_end"),
#        (val_sub, ":cur_banner", arms_meshes_begin),
#        (val_add, ":cur_banner", banner_scene_props_back_begin),
#      (else_try),
#        (assign, ":cur_banner", -1),
#      (try_end),
#      (try_begin),
...
#        (try_end),
#      (try_end),
#      ## options for troop carring banner
#      (try_begin),
#        (eq, ":agent_no", ":player_agent"),
#        (try_begin),
#          (eq, "$g_player_carry_banner", 0),
#          (assign, ":cur_banner", -1),
#        (try_end),
#      (else_try),
#        (eq, "$g_others_carry_banner", 0), # no one
#        (assign, ":cur_banner", -1),
#      (else_try),
#        (eq, "$g_others_carry_banner", 1), # hero
#        (try_begin),
#          (neg|troop_is_hero, ":troop_no"),
#          (assign, ":cur_banner", -1),
#        (try_end),
#      (else_try),
#        (eq, "$g_others_carry_banner", 2), # ally
#        (try_begin),
...
#      (try_end),
#      (gt, ":cur_banner", -1),
#      (agent_get_attached_scene_prop, ":attached_banner", ":agent_no"),
#      (try_begin),
#        (le, ":attached_banner", -1),
#        (scene_prop_get_num_instances, ":instance_no", ":cur_banner"),
#        (spawn_scene_prop, ":cur_banner"),
#        (agent_set_slot, ":agent_no", slot_agent_banner_id, ":cur_banner"),
#        (agent_set_slot, ":agent_no", slot_agent_banner_instance_no, ":instance_no"),
#        (scene_prop_get_instance, ":cur_banner_prop_instance", ":cur_banner", ":instance_no"),
#        (agent_set_attached_scene_prop, ":agent_no", ":cur_banner_prop_instance"),
#        (agent_set_attached_scene_prop_x, ":agent_no", 0),
#        (agent_set_attached_scene_prop_z, ":agent_no", 130),
#      (else_try),
#        (agent_get_slot, ":cur_banner", ":agent_no", slot_agent_banner_id),
#        (agent_get_slot, ":instance_no", ":agent_no", slot_agent_banner_instance_no),
#        (scene_prop_get_instance, ":cur_banner_prop_instance", ":cur_banner", ":instance_no"),
#        (agent_set_attached_scene_prop, ":agent_no", ":cur_banner_prop_instance"),
#        (agent_set_attached_scene_prop_x, ":agent_no", 0),
#        (agent_set_attached_scene_prop_z, ":agent_no", 130),
...
#      (agent_get_attached_scene_prop, ":attached_banner", ":agent_no"),
#      (try_begin),
#        (gt, ":attached_banner", -1),
#        (agent_get_slot, ":cur_banner", ":agent_no", slot_agent_banner_id),
#        (agent_get_slot, ":instance_no", ":agent_no", slot_agent_banner_instance_no),
#        (scene_prop_get_instance, ":cur_banner_prop_instance", ":cur_banner", ":instance_no"),
#        (prop_instance_get_position, pos0, ":cur_banner_prop_instance"),
#        (position_move_z, pos0, -2000),
#        (prop_instance_set_position, ":cur_banner_prop_instance", pos0),
...
custom_commander_hero_wounded_by_player =(
  ti_on_agent_killed_or_wounded, 0, 0, [],
    [
      ...
      (is_between, ":wounded_troop", kings_begin, pretenders_end),
      (try_begin),
        (agent_is_ally, ":dead_agent_no"),
        (call_script, "script_change_player_relation_with_troop", ":wounded_troop", -5),
      (else_try),
        (call_script, "script_change_player_relation_with_troop", ":wounded_troop", -2),
      (try_end),
    ])
  
##Floris: Because this is part of the fix that conflicts with the fix by Caba'drin, I removed this one.  
    ...
    # [
      # (try_for_agents, ":agent_no"),
        ...
        # (try_for_range, ":i_slot", 0, ":end_cond"),
          ...
          # (agent_get_slot,":cur_weapon_1",":agent_no",slot_agent_cur_weapon_1),
          # (agent_get_slot,":cur_weapon_2",":agent_no",slot_agent_cur_weapon_2),
          # (agent_get_slot,":cur_weapon_3",":agent_no",slot_agent_cur_weapon_3),
          # (agent_get_slot,":cur_weapon_4",":agent_no",slot_agent_cur_weapon_4),
          # (try_begin),
            # (eq, ":cur_weapon_1", 0),
            # (agent_set_slot,":agent_no",slot_agent_cur_weapon_1,":item_id"),
          # (else_try),
            # (eq, ":cur_weapon_2", 0),
            # (neq, ":item_id", ":cur_weapon_1"),
            # (agent_set_slot,":agent_no",slot_agent_cur_weapon_2,":item_id"),
          # (else_try),
            # (eq, ":cur_weapon_3", 0),
            # (neq, ":item_id", ":cur_weapon_1"),
            # (neq, ":item_id", ":cur_weapon_2"),
            # (agent_set_slot,":agent_no",slot_agent_cur_weapon_3,":item_id"),
          # (else_try),
            # (eq, ":cur_weapon_4", 0),
            # (neq, ":item_id", ":cur_weapon_1"),
            # (neq, ":item_id", ":cur_weapon_2"),
            # (neq, ":item_id", ":cur_weapon_3"),
            # (agent_set_slot,":agent_no",slot_agent_cur_weapon_4,":item_id"),
          # (try_end),
        # (try_end),
      # (try_end),
    # ])
    
##Floris: Because this fix conflicts with the fix by Caba'drin, I removed this one.
...
#            (try_for_range, ":cur_slot", slot_agent_cur_weapon_1, slot_agent_cur_weapon_4+1),
...
#            (ge, ":horse_agent", 0),
#            (agent_get_wielded_item, ":weapon_item", ":agent_no", 0),
#            (neg|is_between, ":weapon_item", weapons_ranged_begin, weapons_ranged_end),
#            (try_for_range, ":cur_slot", slot_agent_cur_weapon_1, slot_agent_cur_weapon_4+1),
#              (agent_get_slot,":cur_weapon",":agent_no",":cur_slot"),
#              (gt, ":cur_weapon", 0),
#              (is_between, ":cur_weapon", weapons_ranged_begin, weapons_ranged_end),
#              (agent_set_wielded_item, ":agent_no", ":cur_weapon"),
#            (try_end),
#          (try_end),
#        (else_try),
...
##                (neq, ":i_type", itp_type_polearm), ##Floris: Removed, although I have no idea from what mod anymore.
...
##CC 1.324, disabled in Floris because it seems to be bugged.
# Register 0: damage dealer item_id
#custom_commander_critical_strike =(
#  ti_on_agent_hit, 3, 0, [], #1.325: changed 0 to 3
#    [
...
#          (assign, ":proficiency_type", wpt_one_handed_weapon),
#        (else_try),
#          (eq, ":item_type", itp_type_two_handed_wpn),
#          (assign, ":proficiency_type", wpt_two_handed_weapon),
#        (else_try),
#          (eq, ":item_type", itp_type_polearm),
#          (assign, ":proficiency_type", wpt_polearm),
#        (else_try),
#          (eq, ":item_type", itp_type_bow),
#          (assign, ":proficiency_type", wpt_archery),
#        (else_try),
#          (eq, ":item_type", itp_type_crossbow),
#          (assign, ":proficiency_type", wpt_crossbow),
#        (else_try),
#          (eq, ":item_type", itp_type_thrown),
#          (assign, ":proficiency_type", wpt_throwing),
#        (else_try),
#          (this_or_next|eq, ":item_type", itp_type_pistol),
#          (eq, ":item_type", itp_type_musket),
#          (assign, ":proficiency_type", wpt_firearm),
#        (try_end),
#        (agent_get_troop_id, ":dealer_troop", ":dealer_agent"),
#        (store_proficiency_level, ":proficiency_level", ":dealer_troop", ":proficiency_type"),
#        (try_begin),
#          (this_or_next|eq, ":dealer_item_id", "itm_bastard_sword_b"),
#          (this_or_next|eq, ":dealer_item_id", "itm_one_handed_battle_axe_c"),
#          (this_or_next|eq, ":dealer_item_id", "itm_sarranid_two_handed_mace_1"),
#          (this_or_next|eq, ":dealer_item_id", "itm_war_bow"),
#          (this_or_next|eq, ":dealer_item_id", "itm_sniper_crossbow"),
#          (this_or_next|eq, ":dealer_item_id", "itm_morningstar"),
#          (eq, ":dealer_item_id", "itm_khergit_sword_two_handed_a"),
#          (val_add, ":proficiency_level", 100),
#        (try_end),
#        (try_begin),
...
#          (val_add, ":proficiency_level", 200),
#        (try_end),
#        (store_random_in_range, ":r", 0, 1000),
#        (lt, ":r", ":proficiency_level"),
#        # dest damage ratio is 2/3~4/3, added in 1.325
#        (store_mul, ":min_damage", ":inflicted_damage", 2),
#        (store_mul, ":max_damage", ":inflicted_damage", 4),
#        (val_add, ":max_damage", 1),
#        (store_random_in_range, ":dest_damage", ":min_damage", ":max_damage"),
#        (val_div, ":dest_damage", 3),
#        (assign, reg1, ":dest_damage"),
#        (try_begin),
#          (eq, ":dealer_agent", ":player_agent"),
#          (display_message, "@Delivered Critical Strike!", 0xFF0000),
#          (try_begin), #1.325
#            (neg|agent_is_human, ":inflicted_agent"),
#            (agent_get_rider, ":rider_agent", ":inflicted_agent"),
#            (gt, ":rider_agent", -1),
#            (display_message, "@Delivered {reg1} damage to horse."),
#          (else_try),
#            (display_message, "@Delivered {reg1} damage."),
#          (try_end),
#        (else_try),
#          (eq, ":inflicted_agent", ":player_agent"),
#          (display_message, "@Received Critical Strike!", 0xFF0000),
#          (display_message, "@Received {reg1} damage."), #1.325
#        (try_end),
#        (agent_deliver_damage_to_agent, ":dealer_agent", ":inflicted_agent", ":inflicted_damage"),
#        (store_agent_hit_points, ":inflicted_agent_hp", ":inflicted_agent", 1), #1.325
...
dplmc_horse_speed = (
  ...
  (try_for_agents, ":horse_agent"),
	...
	(agent_get_rider, ":agent_no", ":horse_agent"),
	(ge, ":agent_no", 0), ##CABA - flipped the horse/agent loop bit to save two operations per agent per second
...
#Pavise #re-written by Caba  ##CABA - revisit this when WSE is installed to allow destroyable and able to collapse/pick up
## Imported to pbod_mission_templates and implemented there
deploy_pavise = (
    0, 0, 1, [(key_clicked, "$key_special_2")],
       [
		(agent_get_wielded_item, ":shield", "$fplayer_agent_no", 1),
		(gt, ":shield", itm_no_item),
		(store_add, ":end", "itm_sh_rho_pav_deploy_f", 1),
		(this_or_next|eq, ":shield", "itm_sh_pla_pav_eagle"),
		(is_between, ":shield", "itm_sh_rho_pav_deploy_a", ":end"),
		(assign, ":prop", "spr_sh_pla_pav_eagle"),
		(try_begin),
			(is_between, ":shield", "itm_sh_rho_pav_deploy_a", ":end"),
			(store_sub, ":prop", ":shield", "itm_sh_rho_pav_deploy_a"),
			(val_add, ":prop", "spr_sh_rho_pav_deploy_a"),
		(try_end),
		(agent_get_position, pos2, "$fplayer_agent_no"),
		(position_move_y, pos2, 50),
        (set_spawn_position, pos2),
		(spawn_scene_prop, ":prop"),#pavise spawn
        (agent_unequip_item, "$fplayer_agent_no", ":shield"),
       ])
#Pavise
...
custom_commander_commom_triggers = [
    ...
    custom_commander_init_hero_begin_xp,
    custom_commander_give_hero_extra_xp,
##Floris: Update from CC 1.321, and disabled in 1.322
    #custom_commander_init_troop_backup_hp,
    #custom_commander_agent_refill_hp,
    #custom_commander_agent_refill_hp_when_knocked_down,
##
...
#    custom_commander_clear_banners_on_back,
    custom_commander_hero_wounded_by_player,
#    custom_commander_agent_weapons_backup, #Floris: Disabled because it conflicts with Caba'drin's fix.
#    custom_commander_agent_weapons_fix, #Floris: Disabled because it conflicts with Caba'drin's fix.
#    custom_commander_critical_strike, ##CC 1.324, disabled in Floris because it is bugged
    dplmc_horse_speed,
#	deploy_pavise,   #Floris - moved to implementation with PBOD_mission_templates
  ]
## CC
...
common_siege_defender_reinforcement_check = (
  ...
  [
    ...
    (try_begin),
	  ...
	  (lt, "$defender_reinforcement_stage", "$g_reinforcement_stage"), #Floris 2.54 #dunno if it helps??
    ...
    (try_end),
    ## CC
    (try_begin), ##CC ##Floris reworking 2.54
      (ge, "$defender_reinforcement_stage", 3),
      (try_begin), 	  
	    (ge, "$defender_reinforcement_stage", 6),
		(assign, ":grc_target", grc_everyone), #AI desperate charge: everyone!!!
	  (else_try),
	    (assign, ":grc_target", grc_infantry), #AI desperate charge:infantry!!!
	  (try_end),
      (set_show_messages, 0),
	  #Without PBOD, uncomment
	  #(get_player_agent_no, reg0),
	  #(agent_get_team, reg0, reg0),
	  #End without PBOD
	  (try_begin),
	    (neq, "$defender_team", "$fplayer_team_no"), ##PBOD dependency
		#(neq, "$defender_team", reg0), #Without PBOD, uncomment
		(team_give_order, "$defender_team", ":grc_target", mordr_charge), 
	  (try_end),
	  (try_begin),
		(neq, "$defender_team_2", "$fplayer_team_no"), ##PBOD dependency
		#(neq, "$defender_team_2", reg0), #Without PBOD, uncomment
		(team_give_order, "$defender_team_2", ":grc_target", mordr_charge),
	  (try_end),
      (set_show_messages, 1),
    (try_end),
   ])
...
common_battle_check_victory_condition = (
  5, 60, ti_once, ## CC changed check interval to 5, was 1
  [
    (store_mission_timer_a,reg(1)),
    (ge,reg(1),10),
...
#Tempered added for siege camps
	  (try_begin),
...
common_battle_order_panel_tick = (
  ...
  [
    (try_begin),
	  ...
	  (neg|is_presentation_active, "prsnt_caba_camera_mode_display"), #CABA ADD
      (start_presentation, "prsnt_mini_map"),
    (try_end),
    (try_begin),
      (is_presentation_active, "prsnt_battle"),
...
#TEMPERED  DROWNING TRIGGER ASSUMES A WATER LEVEL OF -0.5, TOGGLE IN CAMP ACTIONS MENU TO DISABLE/ENABLE		
common_drowning = (
		6,0,0,[(eq,"$drowning",1)],
  ...
  (1, 4, ti_once, [(eq, "$g_mt_mode", abm_training),
   ...
   [
       ...
       (set_visitor, 35, "trp_veteran_fighter"),
		...
		(set_visitor, 36, ":mercenary_armbrust_komtur"),
		##Floris MTT end
       (set_jump_entry, 50),
...
##Floris: Updated from CC 1.321, but removed in Floris 2.4 since it conflicted with changing to the trade screen.
#    ## CC
#    common_battle_order_panel,
#    common_battle_order_panel_tick,
#    ## CC
...
##
    ...
    [
      ...
      [
		...
		(try_begin),
			...
			(agent_equip_item, ":agent_no", "itm_ar_swa_dip_coatplate_a"),
			(agent_equip_item, ":agent_no", "itm_bo_swa_t5_hose"),
		(else_try),
  ...
  (
    ...
    [(0,mtef_team_0|mtef_use_exact_number,af_override_horse|af_override_weapons|af_override_head,aif_start_alarmed,4,[itm_we_sar_spear_staff_quarter]),
     (3,mtef_visitor_source|mtef_team_1,af_override_horse,aif_start_alarmed,1,[]),
     ],
    [
      common_inventory_not_available,
  ...
  (
    "lead_charge",mtf_battle_mode|mtf_synch_inventory,charge,	#	1.143 Port // mtf_synch_inventory added
    "You lead your men to battle.",
    [
...
#          (str_store_troop_name, s6, ":dead_agent_troop_id"), ##1.131. These five lines are disabled in 1.132
...
##Floris: Disabled these lines because they conflict with the PreBattle Orders.
#                           (get_player_agent_no, ":player_agent"),
#                           (agent_get_team, ":player_team", ":player_agent"),
#                           (set_show_messages, 0),
...
##
                           ...
                           #(assign, "$g_dplmc_charge_when_dead", 0), #Diplomacy 3.32
                           ##diplomacy end                       
                           ]),
  ...
  (
    "village_attack_bandits",mtf_battle_mode|mtf_synch_inventory,charge,	#	1.143 Port // mtf_synch_inventory added
    "You lead your men to battle.",
    [
  ...
  (
    "village_raid",mtf_battle_mode|mtf_synch_inventory,charge,		#	1.143 Port // mtf_synch_inventory added
    "You lead your men to battle.",
    [
  ...
  (
    ...
    [
      ...
      (0, 0, ti_once, [], [(assign,"$g_battle_won",0),
                           ##diplomacy begin
                           #(assign, "$g_dplmc_cam_activated", 0),
                           #(assign, "$g_dplmc_charge_when_dead", 1), #Diplomacy 3.32                                                 
                           ##diplomacy end         
                           (call_script, "script_music_set_situation_with_culture", mtf_sit_ambushed),                       
                           ]),
  ...
  (
    ...
    [
      ...
      (0, 0, ti_once, [], [(assign,"$g_battle_won",0),
                           ##diplomacy begin
                           #(assign, "$g_dplmc_cam_activated", 0),
                           #(assign, "$g_dplmc_charge_when_dead", 1), #diplomacy 3.3.2                                             
                           ##diplomacy end         
                           (call_script, "script_music_set_situation_with_culture", mtf_sit_ambushed),                  
                           ]),
  ...
  (
    "castle_attack_walls_defenders_sally",mtf_battle_mode|mtf_synch_inventory,-1,		#	1.143 Port // mtf_synch_inventory added
    "You attack the walls of the castle...",
    [
  ...
  (
    "castle_attack_walls_belfry",mtf_battle_mode|mtf_synch_inventory,-1,		#	1.143 Port // mtf_synch_inventory added
    "You attack the walls of the castle...",
    [
     ## CC
  ...
  (
    "castle_attack_walls_ladder",mtf_battle_mode|mtf_synch_inventory,-1,		#	1.143 Port mtf_synch_inventory added
    "You attack the walls of the castle...",
    [
     ## CC
     (0,mtef_attackers|mtef_team_1,af_override_horse,aif_start_alarmed,18,[]),
...
#         (tutorial_box, "str_trainer_help_2", "@Tutorial"),
  ...
  (
    ...
    [
      ...
      (19, mtef_visitor_source|mtef_team_3,af_override_everything,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_boots]),
...
#      (ti_before_mission_start, 0, 0, [], [(set_rain, 1,100)]),
  ...
  (
    ...
    [
      (0,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_arena_tunic_red, itm_tourney_helm_red]),
      (1,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy, itm_arena_tunic_red]),
      (2,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_horse,itm_arena_tunic_red, itm_tourney_helm_red]),
      (3,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_red, itm_tourney_helm_red]),
      ...
      (6,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_horse,itm_arena_tunic_red]),
      (7,mtef_visitor_source|mtef_team_0,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_red, itm_tourney_helm_red]),
      ...
      (9,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_blue,itm_tourney_helm_blue]),
      (10,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_arena_tunic_blue]),
      (11,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword,itm_practice_shield,itm_arena_tunic_blue, itm_tourney_helm_blue]),
      ...
      (13,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_blue,itm_tourney_helm_blue]),
      (14,mtef_visitor_source|mtef_team_1,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_arena_tunic_blue]),
      ...
      (16,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_horse,itm_arena_tunic_green, itm_tourney_helm_green]),
      (17,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_arena_tunic_green, itm_tourney_helm_green]),
      (18,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_horse,itm_arena_tunic_green, itm_tourney_helm_green]),
      (19,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_green, itm_tourney_helm_green]),
      (20,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_dagger, itm_arena_tunic_green, itm_tourney_helm_green]),
      ...
      (22,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_horse,itm_arena_tunic_green]),
      (23,mtef_visitor_source|mtef_team_2,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_green, itm_tourney_helm_green]),
      ...
      (25,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_arena_tunic_yellow, itm_tourney_helm_yellow]),
      (26,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_horse,itm_arena_tunic_yellow, itm_tourney_helm_yellow]),
      (27,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_yellow, itm_tourney_helm_yellow]),
      (28,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_bow,itm_practice_arrows,itm_practice_dagger, itm_arena_tunic_yellow, itm_tourney_helm_yellow]),
      ...
      (30,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_sword_heavy,itm_practice_horse,itm_arena_tunic_yellow]),
      (31,mtef_visitor_source|mtef_team_3,af_override_all,aif_start_alarmed,1,[itm_practice_lance,itm_practice_shield,itm_practice_horse,itm_arena_tunic_yellow, itm_tourney_helm_yellow]),
#32
...
#LAZERAS MODIFIED  {spar troops}
  ...
  (
    ...
    "Wedding",
    [
        (0,mtef_visitor_source,af_override_everything,0,1,[itm_ar_swa_tun_tabard, itm_bo_pla_t2_ankle]),
        (1,mtef_visitor_source,af_override_everything,0,1,[itm_dress_general_bride, itm_he_gen_lad_bride_crown, itm_bo_gen_t0_bride_shoes]),
        (2,mtef_visitor_source,af_castle_lord,0,1,[]),
        (3,mtef_visitor_source,af_override_everything,0,1,[itm_ar_pla_pri_bishop, itm_bo_pla_t5_highlander, itm_ga_pla_pri_bishop, itm_he_pla_pri_bishopmitre]), # Wedding Priest. Idea: have a bishop for royal weddings, a normal priest or monk for poor ones.
...
##             (lt, ":sq_dist_2", 400), #2 meters
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only,af_override_everything,0,1,[itm_tutorial_shield,itm_tutorial_sword,itm_tutorial_short_bow,itm_tutorial_arrows,itm_ar_vae_t4_jerkin_a,itm_bo_vae_t3_leather]), #af_override_weapons
     ],
    [
      (ti_tab_pressed, 0, 0, [],
       [(try_begin),
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only|mtef_team_0,af_override_everything,0,1,[itm_tutorial_shield,itm_ar_vae_t4_jerkin_a,itm_bo_vae_t3_leather]),
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only|mtef_team_0,af_override_everything,0,1,[itm_ar_vae_t4_jerkin_a,itm_bo_vae_t3_leather]),
        (3,mtef_visitor_source|mtef_team_1,0,aif_start_alarmed,1,[]),
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only|mtef_team_0,af_override_everything,0,1,[itm_tutorial_staff,itm_ar_vae_t4_jerkin_a,itm_bo_vae_t3_leather]), 
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_leader_only|mtef_team_0,af_override_everything,0,1,[itm_tutorial_sword,itm_tutorial_short_bow,itm_tutorial_arrows,itm_ar_vae_t4_jerkin_a,itm_bo_vae_t3_leather]), #af_override_weapons
     ],
    [
      (ti_tab_pressed, 0, 0, [],
       [(try_begin),
  ...
  (
    ...
    "You enter the training ground.",
    [
        (0,mtef_visitor_source|mtef_team_0,af_override_everything,0,1,[itm_tutorial_sword,itm_tutorial_shield,itm_tutorial_short_bow,itm_tutorial_arrows,itm_tutorial_saddle_horse,itm_ar_vae_t4_jerkin_a,itm_bo_vae_t3_leather]),
        (1,mtef_visitor_source|mtef_team_0,0,aif_start_alarmed,1,[]),
        (2,mtef_visitor_source|mtef_team_0,0,aif_start_alarmed,1,[]),
        (3,mtef_visitor_source|mtef_team_0,0,aif_start_alarmed,1,[]),
        (4,mtef_visitor_source|mtef_team_0,0,aif_start_alarmed,1,[]),
    ...
    [
      ...
      (0, 0, 0, [(try_begin),
				...
				##Floris MTT begin
				   ...
				   (set_visitor,8,":bandit_bandit"),
				   (set_visitor,9,":bandit_bandit"),
				   (set_visitor,10,":bandit_bandit"),
				   (set_visitor,11,":bandit_bandit"),
				##Floris MTT end
    ...
    [
           ...
           (try_begin),         
             ...
             (try_begin),
               ...
               (try_begin),
                 ...
                 (call_script, "script_check_achievement_last_man_standing", "$g_winner_team"), ##1.132, new line
               (else_try),
                 (try_begin),
      ...
      (1, 0, 3, [(multiplayer_is_server),
       ...
       [
         ...
         (try_for_range, ":player_no", 0, ":num_players"),
           (player_is_active, ":player_no"),
       #    (player_set_slot, ":player_no", slot_player_spawned_this_round, 0),		#	1.143 Port // removed
           (player_get_agent_id, ":player_agent", ":player_no"),
           (ge, ":player_agent", 0),
           (agent_is_alive, ":player_agent"),
           (player_save_picked_up_items_for_next_spawn, ":player_no"),
         ...
         (try_for_range, ":player_no", 0, ":num_players"),
           (player_is_active, ":player_no"),		   
		   (player_slot_eq, ":player_no", slot_player_spawned_this_round, 1),		#	1.143 Port // Line added
         ...
         (try_for_range, ":player_no", 0, ":num_players"),							#	1.143 Port // Newly added
           (player_is_active, ":player_no"),
           (player_set_slot, ":player_no", slot_player_spawned_this_round, 0),
         (try_end),																	#	 End
  ...
  (
	"bandit_lair",mtf_battle_mode|mtf_synch_inventory,charge,		#	1.143 Port // mtf_synch_inventory added
    ...
    [
      ...
      (1, 0, ti_once, [],
	   ...
	   [          
          ...
          (assign, ":bandit_troop", -1), ##Floris MTT
				...
				##Floris MTT begin
					(this_or_next|eq, ":template", "pt_looter_lair"),
					(this_or_next|eq, ":template", "pt_looter_lair_r"),
					(this_or_next|eq, ":template", "pt_looter_lair_e"),
				##Floris MTT end
					(neq, ":random_value", 0),
      ...
      (2, 0, ti_once, 
       ...
       [
		...
		##Floris MTT end
         ...
         (else_try),
			##Floris MTT begin
			#(this_or_next|neq|eq, ":template", "pt_looter_lair"), #native line comment out
			(assign, ":break", 0),
			(try_begin),
          ...
          (else_try),  
            ...
            (try_for_range, ":unused", 0, 20),
				...
				(else_try),
					...
					(spawn_around_party, "p_main_party", "pt_looters_r"),
				(else_try),
...
#trigger to spawn unmounted horses in camp, in line formation (requires a spawn point 47 and 48 for start and end of line).
	(0,0,ti_once,[],[  
					...
					(try_for_range,":unused",0,":horse_count"),
						...
						(try_begin),
							(le,":random_no",100),
							(assign,":horse_type","itm_ho_nor_courser_greysteel"),
						(else_try),
							(ge,":random_no",200),
							(assign,":horse_type","itm_ho_pla_sumpter_white"),
						(else_try),
							(is_between,":random_no",100,200),
							(assign,":horse_type","itm_ho_swa_saddle_black"),
						(try_end),
						(set_spawn_position, pos3),
	...
	(0, 0, ti_once,
      ...
      (0, 0, ti_once, [], [(assign,"$g_battle_won",0),
                           ...
                           #(call_script, "script_place_player_banner_near_inventory"),
                           (call_script, "script_combat_music_set_situation_with_culture"),
                           (assign, "$g_defender_reinforcement_limit", "$g_reinforcement_stage"), ##CC
                           ]),
    ...
    [
      (ti_on_agent_spawn, 0, 0, [], [
         ...
         (try_begin),
           (eq, ":team", 0), # Player
           (try_begin),
             (eq, "$g_lieutenant_sparring_weapon", 0),
             (agent_equip_item, ":agent", "itm_practice_sword_heavy"),
             (agent_set_wielded_item, ":agent", "itm_practice_sword_heavy"),
           (else_try),
             (eq, "$g_lieutenant_sparring_weapon", 1),
             (agent_equip_item, ":agent", "itm_practice_sword"),
             (agent_equip_item, ":agent", "itm_practice_shield"),
             (agent_set_wielded_item, ":agent", "itm_practice_sword"),
           (else_try),
             (eq, "$g_lieutenant_sparring_weapon", 2),
             (agent_equip_item, ":agent", "itm_practice_staff"),
             (agent_set_wielded_item, ":agent", "itm_practice_staff"),
           (else_try),
         ...
         (else_try),
           ...
           (store_proficiency_level, ":wp_one_hand", ":troop_id", wpt_one_handed_weapon),
           (store_proficiency_level, ":wp_two_hand", ":troop_id", wpt_two_handed_weapon),
           (store_proficiency_level, ":wp_polearm", ":troop_id", wpt_polearm),
           (store_proficiency_level, ":wp_archery", ":troop_id", wpt_archery),
           (store_add, ":total_wp", ":wp_one_hand", ":wp_two_hand"),
           (val_add, ":total_wp", ":wp_polearm"),
           (val_add, ":total_wp", ":wp_archery"),
           (try_begin),
             (le, ":total_wp", 0),
             (assign, ":total_wp", 1), # Failsafe
           (try_end),
           (store_random_in_range, ":rand", 0, ":total_wp"),
           (try_begin),
             (lt, ":rand", ":wp_two_hand"),
             (agent_equip_item, ":agent", "itm_practice_sword_heavy"),
             (agent_set_wielded_item, ":agent", "itm_practice_sword_heavy"),
           (else_try),
             (val_sub, ":rand", ":wp_two_hand"),
             (lt, ":rand", ":wp_one_hand"),
             (agent_equip_item, ":agent", "itm_practice_sword"),
             (agent_equip_item, ":agent", "itm_practice_shield"),
             (agent_set_wielded_item, ":agent", "itm_practice_sword"),
           (else_try),
             (val_sub, ":rand", ":wp_one_hand"),
             (lt, ":rand", ":wp_polearm"),
             (agent_equip_item, ":agent", "itm_practice_staff"),
             (agent_set_wielded_item, ":agent", "itm_practice_staff"),
           (else_try),
      ...
      (ti_before_mission_start, 0, 0, [], [
         ...
         (replace_scene_props, "spr_container_small_chest", "spr_chest_b"),
         (replace_scene_props, "spr_container_chest_b", "spr_chest_b"),
         (replace_scene_props, "spr_container_chest_c", "spr_chest_c"),
      ...
      (ti_question_answered, 0, 0, [],
        [
          ...
          (try_for_agents, ":agent"),
             ...
             (agent_get_team, ":team", ":agent"),
             (eq, ":team", 1),
             (val_add, ":enemies_alive", 1),
          (try_end),
          (assign, "$g_lieutenant_sparring_enemies_alive", ":enemies_alive"),
