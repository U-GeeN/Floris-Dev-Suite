  ...
  ("stack_econ_debug_report_stack",
   [
     ...
     (try_begin),
       ...
       (str_store_string, s3, "@all right"),
       (try_begin),
         (gt, ":armor_low_total", ":armor_high"),
         (str_store_string, s3, "@ill"),
       (else_try),
       ...
       (str_store_string, s2, "@For {reg0} men, we have {reg1} denars set aside for equipment and loot worth about {reg2}. The company is mostly {s3} equipped."),
       (assign, reg0, ":armor_high"),
       (assign, reg1, ":armor_average"),
       (assign, reg2, ":armor_low_total"),
       (str_store_string, s2, "@{s2}^For armor, {reg0} men are properly fitted with the best gear we can issue, {reg1} have a mixed kit, and {reg2} are still making do with poor gear."),
       (assign, reg0, ":weapon_1_count"),
       (assign, reg1, ":weapon_average"),
       (assign, reg2, ":weapon_1_low"),
       (str_store_string, s2, "@{s2}^As for weapons, {reg0} men carry good arms, {reg1} have serviceable mixed weapons, and {reg2} still need better weapons."),
     (try_end),
   ]),
  ...
  ("stack_econ_process_stack",
   [
     ...
     (try_begin),
       ...
       # Gold income removed: loot items are now distributed directly via script_stack_econ_distribute_loot.
