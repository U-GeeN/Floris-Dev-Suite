  ...
  (24,
   [
	...
	(try_for_range, ":stack_no", 0, ":range"),
		...
		(assign, ":required_experience", 2000000),
		(store_character_level, ":troop_level", ":troop_no"),
		(try_begin),
			(is_between, ":troop_level", 37, 43), # tier 7
			(assign, ":required_experience", 1000000),
		(try_end),
		(troop_get_xp, ":experience", ":troop_no"),
		(ge, ":experience", ":required_experience"), 
  ...
  (24,
   [
      (try_for_parties, ":party_no"),
          ...
          (try_begin),
              (eq, ":party_no", "p_main_party"),
              (call_script, "script_stack_econ_debug_report_party", ":party_no"),
          (try_end),
      (try_end),
   ]),
