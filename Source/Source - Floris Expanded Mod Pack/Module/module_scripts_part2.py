...
#TEMPERED#    BONUSES END 	
    ...
    ("party_calculate_loot",
      [
        ...
        (try_end),
        (assign, ":regular_troop_shares", 0),
        (party_get_num_companion_stacks, ":main_num_stacks", "p_main_party"),
        (try_for_range, ":main_stack", 1, ":main_num_stacks"),
          (party_stack_get_troop_id, ":main_troop", "p_main_party", ":main_stack"),
          (neg|troop_is_hero, ":main_troop"),
          (party_stack_get_size, ":main_stack_size", "p_main_party", ":main_stack"),
          (val_add, ":regular_troop_shares", ":main_stack_size"),
        (try_end),
        (assign, ":troop_share_loot_probability", 0),
        (try_begin),
          (gt, ":regular_troop_shares", 0),
          (store_mul, ":troop_share_loot_probability", ":regular_troop_shares", 3),
          (val_mul, ":troop_share_loot_probability", "$g_strength_contribution_of_player"),
          (val_mul, ":troop_share_loot_probability", ":player_party_looting"),
          (val_div, ":troop_share_loot_probability", 10),
          (val_div, ":troop_share_loot_probability", ":num_player_party_shares"),
          (val_max, ":troop_share_loot_probability", 1),

          (call_script, "script_copy_inventory", "trp_temp_troop", "trp_temp_array_a"),
          (troop_clear_inventory, "trp_temp_troop"),
          (assign, "$g_stack_econ_filter_direct_to_pool", 1),
          (assign, "$g_stack_econ_loot_distribution_silent", 1),
          (party_get_num_companion_stacks, ":num_stacks", ":enemy_party"),
          (try_for_range, ":i_stack", 0, ":num_stacks"),
            (party_stack_get_troop_id, ":stack_troop", ":enemy_party", ":i_stack"),
            (neg|troop_is_hero, ":stack_troop"),
            ...
            (try_for_range, ":unused", 0, ":stack_size"),
              (troop_clear_inventory, "trp_temp_troop"),
              (troop_loot_troop, "trp_temp_troop", ":stack_troop", ":troop_share_loot_probability"),
              (call_script, "script_stack_econ_filter_casualty_loot", ":stack_troop"),
              (assign, ":casualty_hidden_items", 0),
              (troop_get_inventory_capacity, ":hidden_inv_cap", "trp_temp_troop"),
              (try_for_range, ":hidden_slot", 0, ":hidden_inv_cap"),
                (troop_get_inventory_slot, ":hidden_item", "trp_temp_troop", ":hidden_slot"),
                (ge, ":hidden_item", 0),
                (val_add, ":casualty_hidden_items", 1),
              (try_end),
              (try_begin),
                (gt, ":casualty_hidden_items", 0),
                (call_script, "script_stack_econ_distribute_loot", "p_main_party"),
                (assign, ":casualty_loot_value", reg3),
                (assign, ":casualty_result_count", reg4),
                (try_begin),
                  (eq, stack_econ_debug_messages, 1),
                  (str_store_troop_name, s10, ":stack_troop"),
                  (assign, reg0, ":casualty_hidden_items"),
                  (assign, reg1, ":casualty_loot_value"),
                  (try_begin),
                    (gt, ":casualty_result_count", 0),
                    (display_message, "@DEBUG: {s10} looted {reg0} item(s) and {reg1} loot value. Result: {s51}", 0xFF99DDFF),
                  (else_try),
                    (display_message, "@DEBUG: {s10} looted {reg0} item(s) and {reg1} loot value.", 0xFF99DDFF),
                  (try_end),
                (try_end),
              (try_end),
            (try_end),
          (try_end),
          (assign, "$g_stack_econ_filter_direct_to_pool", 0),
          (assign, "$g_stack_econ_loot_distribution_silent", 0),
          (assign, reg50, 0),
          (troop_clear_inventory, "trp_temp_troop"),
