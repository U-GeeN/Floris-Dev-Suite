...
##diplomacy end+
  ...
  [anyone,"horse_manage_inventory", [
    ...
    (try_begin),
      (ge, reg0, 0),
      (assign, "$g_horse_manage_original_troop", reg0),
      (assign, "$g_horse_manage_mounted_mode", 1),
    (try_end),
    (call_script, "script_horse_assignment_store_management_status", "p_main_party", "$g_horse_manage_original_troop"),
  ], "{s2}", "horse_manage_select", []],
] + [
  [anyone|plyr,"horse_manage_select", [
    (call_script, "script_horse_assignment_find_management_option", "p_main_party", "$g_horse_manage_original_troop", horse_index),
    (ge, reg0, 0),
...
####################################################################################################################################
 ##Caba - new cheats
  ...
  [anyone|plyr,"regular_member_talk", [(ge, "$cheat_mode", 1)], "CHEAT: Wound One Member", "do_regular_member_view_char",[
    (call_script, "script_stack_econ_find_stack_by_troop", "p_main_party", "$g_talk_troop"),
    (assign, ":stack", reg0),
    (neq, ":stack", -1),
    (party_stack_get_size, ":stack_size", "p_main_party", ":stack"),
    (party_stack_get_num_wounded, ":wounded_count", "p_main_party", ":stack"),
    (try_begin),
        (gt, ":stack_size", ":wounded_count"),
        (party_wound_members, "p_main_party", "$g_talk_troop", 1),
        (display_message, "@CHEAT: Wounded one member of this stack.", 0xFF66FF66),
    (else_try),
        (display_message, "@CHEAT: All members of this stack are already wounded.", 0xFFFF6666),
    (try_end),
   ]],
  [anyone|plyr,"regular_member_talk", [(ge, "$cheat_mode", 1)], "CHEAT: Upgrade One Member", "do_regular_member_view_char",[
    ...
    (try_begin),
        ...
        (assign, ":xp_to_add", reg0),
        (party_add_xp_to_stack, "p_main_party", ":stack", reg0),
        (call_script, "script_stack_econ_initialize_stack", "p_main_party", ":stack"),
        (call_script, "script_stack_econ_get_field", "p_main_party", ":stack", stack_econ_field_tracked_xp),
        (assign, ":tracked_xp", reg1),
        (val_add, ":tracked_xp", ":xp_to_add"),
        (call_script, "script_stack_econ_set_field", "p_main_party", ":stack", stack_econ_field_tracked_xp, ":tracked_xp"),
    (else_try),
  ...
  [anyone|plyr,"regular_member_talk", [(ge, "$cheat_mode", 1)], "CHEAT: Equip One Member for Next Upgrade", "do_regular_member_view_char",[
    ...
    (try_end),
    (neq, ":stack", -1),
    (call_script, "script_stack_econ_cheat_equip_stack_for_next_upgrade", "p_main_party", ":stack", 1),
    (call_script, "script_stack_econ_refresh_visible_troop_agents", "$g_talk_troop"),
   ]],
  [anyone|plyr,"regular_member_talk", [(ge, "$cheat_mode", 1)], "CHEAT: Equip Full Stack for Next Upgrade", "do_regular_member_view_char",[
    (assign, ":stack", -1),
    (party_get_num_companion_stacks, ":num_of_stacks", "p_main_party"),
    (try_for_range, ":i", 0, ":num_of_stacks"),
        (party_stack_get_troop_id, ":stack_troop", "p_main_party", ":i"),
