...
# Refresh Merchants
  (0.0, 0, 168.0, [],
  [    
    (call_script, "script_refresh_center_inventories"),			#	1.143 Port // moved all to script
  ]),

# Refresh Armor sellers
  (0.0, 0, 168.0, [],
  [    
    (call_script, "script_refresh_center_armories"),			#	1.143 Port // moved all to script
  ]),

# Refresh Weapon sellers
  (0.0, 0, 168.0, [],
  [
    (call_script, "script_refresh_center_weaponsmiths"),		#	1.143 Port // moved all to script
  ]),

# Refresh Horse sellers
  (0.0, 0, 168.0, [],
  [
    (call_script, "script_refresh_center_stables"),				#	1.143 Port // moved all to script
  ]),
...
# Troublesome bandits
  (0.3, 0.0, 1.1, [(check_quest_active, "qst_troublesome_bandits"),
                   (neg|check_quest_failed, "qst_troublesome_bandits"),
					...
					(else_try),
						(eq, "$troop_trees", troop_trees_1),
						(store_num_parties_destroyed, ":cur_eliminated", "pt_troublesome_bandits_r"),
						(store_num_parties_destroyed_by_player, ":cur_eliminated_by_player", "pt_troublesome_bandits_r"),
					(else_try),
						(eq, "$troop_trees", troop_trees_2),
						(store_num_parties_destroyed, ":cur_eliminated", "pt_troublesome_bandits_e"),
						(store_num_parties_destroyed_by_player, ":cur_eliminated_by_player", "pt_troublesome_bandits_e"),
					(try_end),
					##Floris MTT end
				   # FLORIS 2.5: Windy+ - Bugfix to prevent auto-failure of the quest.  These conditional statements need to be outside of the MTT try block.
				   (lt, "$qst_troublesome_bandits_eliminated", ":cur_eliminated"),                       # Someone has killed this type of bandit since quest start.
                   (eq, ":cur_eliminated_by_player", "$qst_troublesome_bandits_eliminated_by_player"),   # The player has NOT killed this type of bandit since quest start.
				   # FLORIS 2.5: Windy-
                   ],
                  [(display_message, "str_bandits_eliminated_by_another"),
                   (call_script, "script_abort_quest", "qst_troublesome_bandits", 0),			#	1.143 Port // changed from 2 to 0
                   ]),
  ...
  (0.3, 0.0, 1.1, [(check_quest_active, "qst_troublesome_bandits"),
                   (neg|check_quest_succeeded, "qst_troublesome_bandits"),
					...
					(try_end),
					##Floris MTT end
					# FLORIS 2.5: Windy+ - Bugfix to prevent eroneous successes of the quest.  These conditional statements need to be outside of the MTT try block.
					(lt, "$qst_troublesome_bandits_eliminated", ":cur_eliminated"),                          # Someone has killed this type of bandit since quest start.
					(neq, ":cur_eliminated_by_player", "$qst_troublesome_bandits_eliminated_by_player"),     # The player HAS killed this type of bandit since quest start.
					# FLORIS 2.5: Windy-
                   ],
...
#Check for new personality clashes
			...
			(else_try), 
               ...
               (try_end),
				...
				##Floris MTT begin
				(troop_get_slot,":mercenary_armbrust_miliz","$troop_trees",slot_mercenary_armbrust_miliz),
				(troop_set_slot, ":mercenary_armbrust_miliz", slot_troop_mission_object, ":npc"),
				(assign, "$npc_to_rejoin_party", ":mercenary_armbrust_miliz"),
				##Floris MTT end
        ##diplomacy end
				(else_try), 
					...
					(this_or_next|neg|troop_slot_eq, ":npc", slot_troop_current_mission, npc_mission_rejoin_when_possible),
						(hero_can_join), ##CABA - bugfix was (hero_can_join, ":npc") but the operation takes a partyID, not a troopID
...
# Lady of the lake achievement					#	1.143 Port // Newly Added
   (1, 0, 0,
   [
     ...
     (try_for_range, ":companion", companions_begin, companions_end),
       ...
       (try_for_range, ":i_slot", 0, ":inv_cap"),
		 ...
		 (ge, ":item_id", 0),
	 	 ...
	 	 (this_or_next|eq, ":item_id", "itm_we_swa_sword_twohanded_claymore"),
	 	 (this_or_next|eq, ":item_id", "itm_we_rho_sword_great_long"),
	 	 (this_or_next|eq, ":item_id", "itm_we_pla_sword_great_four"),
		 (eq, ":item_id", "itm_we_pla_sword_strange_great"),
...
(36, 0, 0.0, [], [  ## Set the reinforcements interval to 36 Game hours. Change as you feel like.
                    (try_for_range, ":center", walled_centers_begin, walled_centers_end),
					 (party_get_slot, ":besieged", ":center", slot_center_is_besieged_by),
                        ...
                        (faction_get_slot, ":party_template_f", ":faction", slot_faction_reinforcements_f),
                        ...
                        (try_begin),
                            ...
                            (try_for_range, ":village_reinforcements", villages_begin, villages_end),
                                (try_begin),
                                    ...
                                    (else_try),
                                        (is_between, ":rand", 45, 60), ## Get stronger template
                                        (party_add_template, ":result", ":party_template_c"),
                                    (else_try),
                                        (is_between, ":rand", 60, 75), ## Get stronger template
                                        (party_add_template, ":result", ":party_template_d"),
                                    (else_try),
                                        (is_between, ":rand", 75, 85), ## Get stronger template
                                        (party_add_template, ":result", ":party_template_e"),
                                    (else_try),
                                        (ge, ":rand", 85), ## Get strongest template
                                        (party_add_template, ":result", ":party_template_f"),
                                    (try_end),
